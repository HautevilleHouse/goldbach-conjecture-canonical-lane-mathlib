/-!
# Source package model for `goldbach-conjecture-canonical-lane`
...
def sourceCheckoutHead : String := "f57fb36cfd94773b0f90d2a72a210a85cb2b6ade"
...
def sourceFiles : List SourceFile := [
  ...
  { path := "scripts/gb_closure_guard.py", sha256 := "51140455ded4c296b8c13bac2e23c6278acf0cf92a742723f37a261420afa0c3", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true },
...
def sourceFunctions : List SourceFunctionDecl := [
  ...
  { file := "scripts/gb_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 31, isAsync := false },
  { file := "scripts/gb_closure_guard.py", name := "_resolve", args := ["path_str"], returns := "Path", doc := "", line := 35, isAsync := false },
  { file := "scripts/gb_closure_guard.py", name := "_bootstrap_registry", args := ["path"], returns := "None", doc := "", line := 42, isAsync := false },
  { file := "scripts/gb_closure_guard.py", name := "_load_registry", args := ["path"], returns := "dict[str, Any]", doc := "", line := 58, isAsync := false },
  { file := "scripts/gb_closure_guard.py", name := "_load_sigma_from_stitch", args := ["path"], returns := "float | None", doc := "", line := 70, isAsync := false },
  { file := "scripts/gb_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "tuple[float | None, bool]", doc := "", line := 87, isAsync := false },
  { file := "scripts/gb_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict_coh_zero"], returns := "dict[str, Any]", doc := "", line := 98, isAsync := false },
  { file := "scripts/gb_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "None", doc := "", line := 187, isAsync := false },
  { file := "scripts/gb_closure_guard.py", name := "main", args := [], returns := "None", doc := "", line := 199, isAsync := false },
...
def sourceAssignments : List SourceAssignmentDecl := [
  ...
  { file := "scripts/gb_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 13 },
  { file := "scripts/gb_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 14 },
  { file := "scripts/gb_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 16 },
  { file := "scripts/gb_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 17 },
  { file := "scripts/gb_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 18 },
  { file := "scripts/gb_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 19 },
  { file := "scripts/gb_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_major_arc\", \"sigma_minor_arc\", \"kappa_compact\", \"rho_rigidity\", \"prime_pair_transfer\"]", line := 21 },
  { file := "scripts/gb_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 28 },
...
def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_major_arc", status := "derived_numeric", formula := "c_major_raw * arc_density_raw - e_major_raw", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A", validation := "required_positive", componentKeys := ["arc_density_raw", "c_major_raw", "e_major_raw"] },
  { group := "constants", key := "sigma_minor_arc", status := "derived_numeric", formula := "minor_arc_floor_raw - decomposition_loss_raw - truncation_loss_raw", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B", validation := "required_positive", componentKeys := ["decomposition_loss_raw", "minor_arc_floor_raw", "truncation_loss_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "prime_pair_transfer", status := "derived_numeric", formula := "c_pair_raw * rigidity_transfer_raw - e_pair_raw", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4", validation := "required_positive", componentKeys := ["c_pair_raw", "e_pair_raw", "rigidity_transfer_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Appendix E.6", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Appendix E.6" },
  { key := "kappa_compact", value := "0.8058017727639", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C" },
  { key := "kappa_major_arc", value := "1.0879999999999999", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_major_arc", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "prime_pair_transfer", value := "1.0349", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#prime_pair_transfer", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "rho_rigidity", value := "1.073", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D" },
  { key := "sigma_minor_arc", value := "1.0619999999999998", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_minor_arc", sourceSection := "paper/GOLDBACH_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B" }
]
